import { RecSection } from "./.rec/page";
import { DialAppSection } from "./DialApp/page";
import { ToolsSection } from "./tools/page";
import { SpeedSwitchSection } from "./speedswitch/page";

export default function Page() {
	return <>
	<SpeedSwitchSection/>
	<DialAppSection/>
	<RecSection/>
	<ToolsSection/>
	{/* <section className="Pad vFillView hCenter vCenter flex">
		<div className="vCenter">
			<h1 className="light">We are currently working on our first product</h1>
			<br/>
			<p>For consultation and projects, email us via <a href="mailto:service@zentru.systems" className="hover-fg fg-l2 decorationC-l4 hoverUnderlineAnimation">service@zentru.systems</a></p>
		</div>
	</section> */}
	</>;
}