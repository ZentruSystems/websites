# SpeedSwitch landing page

Route: `/products/speedswitch`. The onboarding page the app opens on first launch lives at
`app/[locale]/products/speedswitch/welcome`.
...
All text lives in `messages/en.ts` and `messages/de.ts` under `Products.speedswitch`, like
...
1. Drop the file into `public/img/speedswitch/` (that folder's README lists the slots).
...
redirects to `speedswitch://claim?src=<token>&v=1`.
...
- **The app side of the rename.** The product was called Transmission, then ShiftDown, before
  it became SpeedSwitch, and three of the names above are contracts the app has to match —
  see below.

## What the rename to SpeedSwitch needs from the app
...
| Here | The app must |
|---|---|
| `speedswitch://claim?src=…&v=1` in `welcome/WelcomeClient.tsx` | Register `speedswitch://` as its URL scheme. Until it does, the claim redirect goes nowhere and `install_confirmed` is the only signal that survives. |
| `/products/speedswitch/welcome` | Open that URL on first launch instead of the `transmission` or `shiftdown` one. |
| `systems.zentru.speedswitch` (the bundle ID in `config.ts`) | Confirm it — the comment there assumes the app was renamed too. |

Nothing already in the wild breaks: the page has not shipped, so no browser holds a
`transmission.src` or `shiftdown.src` token and no installed build is looking for an old
scheme. Doing the renames before launch is what keeps it that way.
